...
def create_video(image_folder='./images', video_name='./video.avi'):
...
def create_gif(image_folder='./images', gif_name='./image.gif'):
    image_names = [name for name in os.listdir(image_folder) if name.endswith(".png")]
    img_array = [imageio.imread(os.path.join(image_folder, name)) for name in image_names]
    imageio.mimwrite(gif_name, img_array, format='GIF-PIL', fps=50, loop=1)
...
def main():
    ...
    out_path = os.path.join(args.save_dir,
                            ...
                            measure_config['noise']['name'],
                            str(measure_config['noise']['sigma']),
                            # 'seed_{}'.format(args.seed)
                            )
    os.makedirs(out_path, exist_ok=True)
    for img_dir in ['input', 'recon', 'progress', 'truth', 'video', 'gif']:
    ...
    for i, ref_img in enumerate(loader):
        ...
        y_n = y_x + noiser.sigma * torch.randn_like(y_x)  # añade ruido gausiano
                                                          # y por tanto nunca añadirá poisson

        # General-Purpose Posterior Sampling via DMPS
        DMPS_start_time = time.time()
        x_start = torch.randn(ref_img.shape, device=device).requires_grad_()
        sample = sample_fn(x_start=x_start, measurement=y_n, H_funcs=H_funcs,
                           noise_std=noiser.sigma, record=args.record,
                           save_root=out_path)
        ...
        if args.record:
            image_folder = os.path.join(out_path, 'progress')
            file_name = str(i).zfill(5)
            if args.video:
                create_video(image_folder=image_folder,
                             video_name=os.path.join(out_path, 'video', file_name + '.avi'))
            create_gif(image_folder=image_folder,
                       gif_name=os.path.join(out_path, 'gif', file_name + '.gif'))
    ...
    print('Total # imges:{}, Sigma: {}, total running Time: {}'.format(Num_count,
                                                                       measure_config['noise']['sigma'],
                                                                       running_time))
